      ...
      if (job.name === 'blog-generate') {
        ...
        const photosWithUrls = await Promise.all(
          photos.map(async (p) => ({
            ...
            lat: p.lat,
            lng: p.lng,
            scene_label: p.sceneLabel,
          })),
        );

        await this.gpuClient.callBlogGenerate({
          job_id: processingJobId,
          photos: photosWithUrls,
          callback_url: `${callbackBase}/internal/jobs/${processingJobId}/blog-callback`,
          ...(job.data.persona ? { persona: job.data.persona } : {}),
        });
      } else {
        const photos = await this.prisma.photo.findMany({
          where: { id: { in: photoIds }, roomId },
